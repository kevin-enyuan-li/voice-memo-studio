...
- **🔇 Reduce Background Noise** — on by default, this runs the same multiband noise-gate worklet used by the effects rack's Noise Reduce effect (see below) live on what's actually being captured, not just as an optional after-the-fact effect. It's wired into the capture graph only — never into the separate Monitor graph — so it can add latency to the recorded file without ever adding latency to what you hear while recording. The effects rack's own Noise Reduce (with its adjustable Strength) is still there for further/different tuning on top.
- **🎙️ Mic device picker** above the record bar lists available input devices (`enumerateDevices()`), used as the `deviceId` constraint on the next recording. Device labels are blank until mic permission has been granted at least once (browser privacy restriction), so the list re-populates with real names right after your first successful recording; it also live-updates on `devicechange` (e.g. plugging in a USB mic or headset).
- **Live level meter** and **elapsed-time counter** while recording. A separate peak detector on the same analyser watches for actual digital clipping (samples at or above ~98% of full scale, not just a loud average) and flashes a **⚠ Clipping** warning — held visible for a moment after the last over so a brief peak doesn't just flash for one frame — since clipping is permanent distortion that can't be fixed after the fact the way background noise can. Lower Input Gain (or back off from the mic) until it stops.
...
- **📝 Live Transcribe** (toggle in the recording panel) — runs `SpeechRecognition` alongside `MediaRecorder` while you talk, showing live text as you go and attaching the finished transcript to the memo in an editable textarea below the waveform (with 📋 Copy and 💾 Export .txt, the latter reusing the same share-or-download path as audio exports). **Chrome/Edge only** — Safari has never implemented the recognition side of the Web Speech API, on iOS or Mac — so the toggle is feature-detected and stays hidden entirely where unsupported, same pattern as the output-device picker. Because `SpeechRecognition` only ever listens to a live mic the browser manages internally, transcription is live-recording-only — there's no way to transcribe a loaded file or an older library entry after the fact. Recognition auto-restarts if the engine stops itself after a silence gap (some do, even with `continuous: true`), and correctly pauses/resumes alongside the recording without losing what's already been transcribed. Saved library entries carry their transcript with them and show a preview snippet in the list.
  - **Speech language** dropdown next to the toggle picks what language *you're speaking* (English, Chinese — Mandarin Simplified/Traditional or Cantonese, Spanish, and a dozen others) — this is independent of your browser's own UI language, which is all `SpeechRecognition` would otherwise default to. Defaults to a smart guess from `navigator.language` (exact match, then base-language match, e.g. a `zh-SG` browser locale still defaults to `zh-CN`) but is always overridable, and persists across sessions.
- **🌐 Translate** into one of ~15 languages (including English) using Chrome's **on-device `Translator` API** — no server, no API key, nothing leaves the browser. This is considerably more experimental than `SpeechRecognition`: it's Chrome-only, may need a flag enabled depending on your Chrome version, and can require downloading a language-pack model on first use of a given language pair (shown as a progress status). Feature-detected the same way — the control is entirely absent if `Translator` isn't available. Translating an unsupported language pair, or a pair the device can't download, surfaces a clear status message rather than failing silently. The translation is itself editable, with its own Copy/Export .txt, and round-trips through the saved-memo library alongside the transcript.
  - **Live Translate** (toggle in the recording panel, next to Transcribe) — translates each sentence the moment `SpeechRecognition` finalizes it, so a rough translation streams in almost as fast as the transcript itself, instead of waiting until the recording stops to translate the whole thing at once. Reuses a single `Translator` instance for the whole recording (rather than re-creating the model per sentence), and a translation queue keeps sentences in the order they were spoken even though each on-device `translate()` call can resolve at its own pace. The finished result lands in the same editable Translation box you get after stopping, and the same "Translate" button is still there to re-run translation over the whole transcript by hand (e.g. after editing it, or after switching target languages) if you didn't have Live Translate on. Transcript and translation are each kept one sentence per line, so **🔀 Interleave** (in the Transcript header) can pair them up sentence-by-sentence — like subtitles — instead of two separately-scrolling boxes; it re-derives the pairing from whatever's currently in the two boxes each time you open it, so edits are reflected, and if the line counts ever don't match (hand-edited, or translated via the one-shot button instead of live) it pairs up as far as it can and lists the rest below rather than guessing.

### Multi-track mixdown
- Once a memo is loaded/recorded, a **🎚️ Tracks** panel appears above the waveform listing it as the primary track. **🎙 Record Another Track** or **📁 Import Track** (accepts the same file types as the main drop zone, multi-select supported) queue up additional clips alongside it.
- Each queued track gets its own **gain slider** and **mute** toggle, plus a **✕** to remove it before merging; the primary track itself is always included at unity gain.
- **🔀 Merge Tracks** sums everything into one buffer via an `OfflineAudioContext` — all tracks start at `0:00` (an overlay merge, not a timeline with per-track offsets) — then hands the result to the normal single-track pipeline as a fresh memo: waveform, trim, effects, export, and Save to Library all work on the merged audio exactly as they would on any other recording. Differing sample rates and mono/stereo channel counts across tracks are resampled/up-mixed automatically. If the sum would clip, it's peak-limited back down to just under full scale afterward.
- Recording an additional track reuses the same capture pipeline (Input Gain, noise reduction, clipping meter, Pause/Reset) as the main recorder, just without disturbing the memo already loaded — it only replaces the primary track once you actually hit Merge.
...
- **🔊 Output device picker** (next to Bypass, once a memo is loaded) switches which speaker/headphone output the audio plays through, via [`AudioContext.setSinkId()`](https://developer.mozilla.org/en-US/docs/Web/API/AudioContext/setSinkId) — **Chrome/Edge only**; the control is feature-detected and stays hidden entirely in browsers without support (Safari, Firefox), rather than showing something that silently does nothing. Applies to both the main playback context and, if you're monitoring live input, the separate monitor context too.
...
| 🔇 Noise Reduce | Strength | A custom `AudioWorkletProcessor` ("noise-gate") — splits the signal into 6 parallel bandpass bands (150Hz–7kHz) and gates each independently on its own envelope, then sums them back together. A single full-band gate can only act in the *gaps* between words — once your voice opens it, steady hiss/hum riding underneath speech passes straight through untouched. Per-band gating instead keeps attenuating whichever bands are currently noise-dominated while the band(s) actually carrying your voice stay open, so it reduces steady background noise *during* speech too, not just silence. A fixed makeup gain compensates for the mild coloration that comes from summing overlapping bandpass filters. |
