...
- **🌐 Translate** (below the transcript, once one exists) — translates the transcript into one of ~15 languages (including English) using Chrome's **on-device `Translator` API** — no server, no API key, nothing leaves the browser. This is considerably more experimental than `SpeechRecognition`: it's Chrome-only, may need a flag enabled depending on your Chrome version, and can require downloading a language-pack model on first use of a given language pair (shown as a progress status). Feature-detected the same way — the control is entirely absent if `Translator` isn't available. Translating an unsupported language pair, or a pair the device can't download, surfaces a clear status message rather than failing silently. The translation is itself editable, with its own Copy/Export .txt, and round-trips through the saved-memo library alongside the transcript.
